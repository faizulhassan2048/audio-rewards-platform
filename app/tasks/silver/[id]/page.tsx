...
export default function SilverParagraphPage() {
  ...
  // Fetch next paragraph from status
  const getNextFromStatus = async () => {
    try {
      const res = await fetch('/api/tasks/silver/status');
      ...
      if (data?.level_complete) { router.push('/tasks/silver?complete=true'); return null; }
      return null;
    } catch { return null; }
  };

  // Page load - check and redirect if needed
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.alreadyCompleted || data.error === 'Already completed' || data.error === 'AD_REQUIRED') {
        toast.info('⏩ Moving to next...');
        setIsSubmitting(false);
        // Fetch next from status after short delay
        setTimeout(async () => {
          const next = await getNextFromStatus();
